...
function EditBookingPage({ booking, services = [], staffData = [], session }) {
  ...
  const handleUpdate = async () => {
    try {
      ...
      // Get staff ID
      const selectedStaffObj = availableStaff.find(
        (s) => s.name === selectedArtist || s.id.toString() === selectedArtist
      )
      ...
      if (result.success) {
        // Redirect to booking updated confirmation page
        router.push(`/booking-updated/${booking.id}`)
      } else {
    ...
    <div className="min-h-screen bg-gray-50 mt-16 sm:mt-20">
      <div className="max-w-7xl mx-auto p-3 sm:p-4 md:p-6">
        ...
        <div className="mb-4 sm:mb-6">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 text-blue-600 hover:text-blue-800 mb-3 sm:mb-4 text-sm sm:text-base"
            disabled={isSubmitting}
          >
            <ArrowLeft size={18} className="sm:w-5 sm:h-5" />
            Back to Appointments
          </button>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Edit Appointment</h1>
            ...
            <div className="m-4 sm:m-6">
              <div className="bg-blue-100 p-3 sm:p-4 rounded-lg">
                ...
                <div className="text-xs sm:text-sm text-gray-700 space-y-1">
                  <p>
                    <span className="font-medium">Booking #:</span> {booking.id}
                  </p>
                  <p>
                ...
                reservation.time && (
                  <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t">
                    ...
                    <textarea
                      ...
                      disabled={isSubmitting}
                      className="w-full px-3 py-2 bg-white text-gray-900 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-50 disabled:text-gray-500 disabled:cursor-not-allowed text-sm sm:text-base"
                      rows={4}
                ...
                <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t">
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                    <button
                      onClick={() => router.back()}
                      disabled={isSubmitting}
                      className="w-full sm:flex-1 py-3 px-4 border-2 border-gray-300 rounded-lg font-semibold text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={handleUpdate}
                      disabled={!reservation?.date || !reservation?.time || isSubmitting}
                      className={`w-full sm:flex-1 py-3 px-4 rounded-lg font-semibold transition-colors flex items-center justify-center gap-2 text-sm sm:text-base ${
                        reservation?.date && reservation?.time && !isSubmitting
